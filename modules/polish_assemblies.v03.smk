...
if rr > 0:
  ...
  racon_shell_command = "module purge;export MODULEPATH=$MODULEPATH:/apps/DEVEL/modulefiles; module load GCC/9.3.0; module load python3/3.6.5; export PATH=" + config["Racon"]["Racon dir"] + "/scripts:$PATH; export PATH=" + config["Racon"]["Racon dir"] + "/build/bin:$PATH;"
...
if mr > 0:
  ...
  if config["Medaka"]["consensus options"] != None:
    medaka_consensus_opts = config["Medaka"]["consensus options"] 
...
if rr > 0 or mr > 0 or nor or hr >0 or config["Parameters"]["run_purgedups"] == True:
  #  if config["Inputs"]["ONT_reads"] == None and not os.path.exists(config["Inputs"]["ONT_filtered"]):
    if config["Inputs"]["ONT_reads"] == None and config["Inputs"]["ONT_dir"] != None:
      ont_dir = config["Inputs"]["ONT_dir"]
      ont_reads = config["Outputs"]["filtlong_dir"] + "reads.ont.fastq.gz"
    #  if not os.path.exists(ont_reads):
  ...
  if not os.path.exists(config["Finalize"]["Merqury db"]):
    ...
    use rule concat_meryl from preprocess_workflow with:
      ...
      params:
        environment = config["Finalize"]["Merqury environment"],
        kmer = config["Finalize"]["Meryl K"],
        genomescope_path = config["Parameters"]["genomescope_path"],
...
if rr > 0:
  rule racon:
    ...
    params:
      ...
      opts = racon_opts
    log:
      "{directory}/rmp/logs/" + str(date) + ".j%j.{base}.racon{param}.out",
      "{directory}/rmp/logs/" + str(date) + ".j%j.{base}.racon{param}.err",
    ...
    run:
      shell(
        ...
        "{params.racon_env}/scripts/racon_wrapper.py -u {params.opts} -t {threads} reads4racon.fastq.gz {input.mapping} {input.assembly} | grep -v '\[' > {output.polished};"
...
if mr > 0:
  rule medaka:
    ...
    log:
      "{directory}/rmp/logs/" + str(date) + ".j%j.{base}.medaka{param}.out",
      "{directory}/rmp/logs/" + str(date) + ".j%j.{base}.medaka{param}.err",
...
if config["Pilon"]["chunks"] > 1 and pr > 0: 
  rule split_pilon:
    input:
  ...
  run:
    ...
    for genome in input.assembly:
      ...
      shell(
        ...
        "{scripts_dir}fasta-stats.py -f {genome} -s {dir}stats/{outbase}.stats.txt -r {dir}stats/{outbase}.gaps.txt;"
