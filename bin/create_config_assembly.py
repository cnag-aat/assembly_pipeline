...
class CreateConfigurationFile(object):
    ...
    def __init__(self):
        ...
        #FLYE PARAMETERS
        self.flye_env = "/home/devel/talioto/miniconda3/envs/flye-v2.8.3/"
        self.flye_cores = 24	                                                                  #Number of threads to run Flye
        self.flye_pol_it = 2				      			                  #Number of polishing iterations to use with FLYE
        self.other_flye_opts = None                                                               #include here genome size in pipeline											
...
####
    ...
    def register_general(self, parser):
        ...
        general_group.add_argument('--lr-type', dest="lr_type", metavar="lr_type", default=self.lr_type, choices=['pacbio-raw', 'pacbio-corr', 'nano-raw', 'nano-corr', 'subassemblies'],  help='Type of long reads (options are flye read-type options). Default %s' % self.lr_type)
...
####
    ...
    def check_parameters(self,args):
        ...
        if args.run_flye == True or args.run_nextdenovo == True or args.racon_rounds > 0 or args.medaka_rounds > 0 or args.nextpolish_ont_rounds > 0 or args.hypo_rounds > 0:
          ...
          if args.ONT_filtered:
            ...
            args.ONT_filtered = args.filtlong_dir + "reads.ont.filtlong.fastq.gz"

          if not os.path.exists(args.ONT_filtered):
            if args.ONT_dir == None and args.ONT_reads == None:
              parser.print_help()
